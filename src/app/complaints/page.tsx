...
export default async function ComplaintsPage({ searchParams }: { searchParams: Promise<{ status?: string; create?: string }> }) {
  ...
  const params = await searchParams; const page = await getComplaintsPage(session.userId, params.status);
  return <AppShell active="complaints" businessName={page.businessName}><section className="ops-page">
    <header className="ops-header"><div><p className="eyebrow">Customer care</p><h1>Manajemen komplain</h1><p>Ticket, prioritas, penanggung jawab, SLA, dan riwayat tindakan dalam satu tempat.</p></div><Link className="primary-button" href="/complaints?create=1"><Plus size={17}/>Buat ticket</Link></header>
    ...
    <div className="ticket-grid">{page.complaints.length===0?<div className="ops-empty"><LifeBuoy size={28}/><h2>Belum ada komplain</h2><p>Ticket baru akan muncul di sini lengkap dengan batas SLA.</p></div>:page.complaints.map(item=><article className="ticket-card" key={item.id}>
      ...
      <form className="ticket-update-form" action={updateComplaintAction}><input type="hidden" name="complaintId" value={item.id}/><select name="status" defaultValue={item.status}>{Object.values(ComplaintStatus).map(v=><option key={v} value={v}>{label(v)}</option>)}</select><select name="priority" defaultValue={item.priority}>{Object.values(ComplaintPriority).map(v=><option key={v} value={v}>{label(v)}</option>)}</select><select name="assignedToUserId" defaultValue={item.assignedToUser?.id??""}><option value="">Belum ditugaskan</option>{page.users.map(user=><option value={user.id} key={user.id}>{user.name}</option>)}</select><input name="note" placeholder="Catatan tindak lanjut" maxLength={2000}/><button className="small-outline-button" type="submit">Simpan</button></form>
    </article>)}</div>
    {params.create?<div className="ops-modal-backdrop"><form className="ops-modal" action={createComplaintAction}><div className="ops-modal-head"><div><p className="eyebrow">Ticket baru</p><h2>Catat komplain</h2></div><Link href="/complaints" aria-label="Tutup">×</Link></div><label>Judul<input name="title" required maxLength={180}/></label><label>Detail<textarea name="description" rows={5} required maxLength={4000}/></label><div className="form-grid"><label>Kontak<select name="contactId"><option value="">Tanpa kontak</option>{page.contacts.map(c=><option value={c.id} key={c.id}>{c.displayName??c.phoneNumber}</option>)}</select></label><label>Kategori<input name="category" placeholder="Layanan, produk, pengiriman…"/></label><label>Prioritas<select name="priority" defaultValue="NORMAL">{Object.values(ComplaintPriority).map(v=><option value={v} key={v}>{label(v)}</option>)}</select></label><label>Penanggung jawab<select name="assignedToUserId"><option value="">Belum ditugaskan</option>{page.users.map(u=><option value={u.id} key={u.id}>{u.name}</option>)}</select></label></div><button className="primary-button" type="submit">Buat ticket</button></form></div>:null}
  </section></AppShell>;
}
